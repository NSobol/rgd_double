...
export const Coach = ({ coachInfo, seats, setSeats }) => {
  ...
  const getBottomSeats = (coachInfo) => {
    return coachInfo.seats.reduce((sum, seat) => {
      if (seat.available && seat.index % 2 !== 0) {
        return sum + 1;
      } else {
      ...
      <div className={s['info']}>
        ...
        <div className={s['seatsQuantityContainer']}>
          <div>
            ...
            <span className={s['seatsQuantitySmall']}>{coachInfo.coach.available_seats}</span>
          </div>
          <div>
            <span className={s['blockText']}>Верхние </span>
            <span className={s['seatsQuantityBig']}>{getTopSeats(coachInfo)}</span>
          </div>
          <div>
            <span className={s['blockText']}>Нижние </span>
            <span className={s['seatsQuantityBig']}>{getBottomSeats(coachInfo)}</span>
          </div>
        </div>
